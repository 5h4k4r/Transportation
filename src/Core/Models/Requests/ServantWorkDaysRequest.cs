...
public class GetServantOnlinePeriodsRequest : IPagingOptions, IValidatableObject
{
    ...
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        if (ExcludeStartHour is not null && ExcludeEndHour is not null)
            if (ExcludeStartHour > ExcludeEndHour)
                yield return new ValidationResult(
                    $"The field {nameof(ExcludeStartHour)} must be greater than {nameof(ExcludeEndHour)}",
        ...
        if ((ExcludeStartHour is null && ExcludeEndHour is not null) ||
            (ExcludeStartHour is not null && ExcludeEndHour is null))
        {
