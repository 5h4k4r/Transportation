...
public class ServantWorkDaysRepository : IServantWorkDaysRepository
{
    ...
    public async Task<ServantOnlinePeriods> GetServantOnlinePeriods(ulong servantId,
        GetServantOnlinePeriodsRequest model)
    {
        ...
        var startOfExcludedTimeSpan = new DateTime(today.Year, today.Month, today.Day, excludeStartHour ?? 0, 0, 0, 0)
            .ToUniversalTime().TimeOfDay;
        var endOfExcludedTimeSpan = new DateTime(today.Year, today.Month, today.Day, excludeEndHour ?? 0, 0, 0, 0)
            .ToUniversalTime().TimeOfDay;
            ...
            var hours = workDay.Select(workingPeriod =>
            {
                ...
                if (excludeStartHour != null && excludeEndHour != null)
                {
                    var startOfExcludedTime =
                        new DateTime(startAt.Year, startAt.Month, startAt.Day, (int)excludeStartHour, 0, 0, 0)
                            .ToUniversalTime();
                    var endOfExcludedTime =
                        new DateTime(startAt.Year, startAt.Month, startAt.Day, (int)excludeEndHour, 0, 0, 0)
                            .ToUniversalTime();

                    if (startAt.TimeOfDay < endOfExcludedTime.TimeOfDay &&
                        startAt.TimeOfDay > startOfExcludedTime.TimeOfDay)
                        startAt = endOfExcludedTime;

                    if (endAt.TimeOfDay > startOfExcludedTime.TimeOfDay &&
                        endAt.TimeOfDay < endOfExcludedTime.TimeOfDay)
